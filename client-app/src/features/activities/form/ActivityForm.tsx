...
const validate = combineValidators({
    ...
    category: isRequired({message: 'The event title is category'}),
            ...
            <Segment clearing>
                <FinalForm validate={validate} initialValues={activity} onSubmit={handleFinalFormSubmit} render={({handleSubmit, invalid, pristine}) => (
                    <Form onSubmit={handleSubmit} loading={loading}>
                        ...
                        <Button onClick={activity.id ? () => history.push(`/activities/${activity.id}`) : () => history.push('/activities')} disabled={loading} floated='right' type='submit' content='Cancel' />
                    </Form>
                )}>
                </FinalForm>
            </Segment>
